...
import rclpy
from Environments.ros_publisher import ROSPublisher
...
class WalkENV(gym.Env):
    ...
    def __init__(self, render = True, backend = gs.gpu, num_envs = 100, device="cuda", plane = True, t_x = 20, t_y = 50, number_of_lanes=1, number_of_rows=10):
        ...
        boundry1 = self.scene.add_entity(
            morph=gs.morphs.Box(
                size=(0.1, 20, 5),
                pos=(10, 0, 0),
                fixed = True
        ...
        self.slam_reset_client = self.ros_publisher.create_client(
            Reset, 
            '/slam_toolbox/reset'
        )
    ...
    def _get_imu_values(self):
        linear_acceleration, _angular_vel, _ = self.imu.read()
    ...
    def _reset_idx(self, envs_idx):
        ...
        if self.slam_reset_client.service_is_ready():
            ...
            self.slam_reset_client.call_async(request)
    ...
    def reset(self):
        ...
        return self.obs_buf, {
            "camera" : self.cam_forward.render(depth=True)
        } 
    ...
    def step(self, action):
        ...
        rclpy.spin_once(self.ros_publisher, timeout_sec=0.0)
        self.time_step += 1
        self.actions = action.clone()
        data = self.lidar.read()
        lidar_points = data
        lidar_distances = data[1]
        
        # distances = data[1][0]  # shape (H, V)
        # points    = data[0][0]  # shape (N, 3)

        # print(f"Distance grid shape: {distances.shape}")
        # print(f"Z range of points: min={points[:,2].min():.3f} max={points[:,2].max():.3f}")
        # print(f"Robot base height: {self.robot.get_link('base').get_pos()[0,2]:.3f}")
        # print(f"Lidar height = base + 0.3 = {self.robot.get_link('base').get_pos()[0,2] + 0.3:.3f}")
        # print(data[0].shape)
        # print(data[0])
        ...
        # print(self.robot.get_link("base").get_pos().shape)
        # print()
        # self.scene.draw_debug_points(
        #     poss = self.lidar.read()[0][0]
        # )
        ...
        # print(
        #     f"Base Quat: {self.base_quat.shape}, Base Ang Vel: {self.base_ang_vel.shape}, Linear Acc: {self.linear_acc.shape}, Lidar Distances: {lidar_distances.shape}, Base Pos: {self.base_pos.shape}, Base Lin Vel Body: {self.base_lin_vel_body.shape}"
        # )

        self.ros_publisher.publish_all(self.base_pos, self.base_quat, self.base_lin_vel_body, self.base_ang_vel, self.linear_acc, lidar_points)
        # self.ros_publisher.publish_imu(self.base_quat.squeeze(0), self.base_ang_vel.squeeze(0), self.linear_acc.squeeze(0))
        # self.ros_publisher.publish_lidar(lidar_distances= lidar_distances.squeeze(0)[:, 32])
        # self.ros_publisher.publish_odometry(self.base_pos.squeeze(0), self.base_quat.squeeze(0), self.base_lin_vel_body.squeeze(0), self.base_ang_vel.squeeze(0))
        # self.ros_publisher.broadcast_tf(self.base_pos.squeeze(0), self.base_quat.squeeze(0))
